...
class LegalAddress(Base):
    ...
    Индекс = Column(Integer, nullable=False)  
    Регион = Column(String, nullable=False)  
    Город = Column(String, nullable=False)   
    Улица = Column(String, nullable=False)   
    Дом = Column(Integer, nullable=False)    
...
class OrderAttachment(Base):
    ...
    id = Column(Integer, primary_key=True)
    order_id = Column(Integer, ForeignKey('заказ.id'))
    file_path = Column(String)
    
    заказ = relationship("Order")
